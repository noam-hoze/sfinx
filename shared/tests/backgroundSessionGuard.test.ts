...
describe("backgroundSessionGuard", () => {
  ...
  it("Timebox: should transition when elapsed >= TIMEBOX_MS", () => {
    const t0 = 1_000_000;
    setNow(t0);
    let gs: GuardState = { zeroRuns: 0, projectsUsed: 0 };
  ...
  it("Projects cap: triggers when projectsUsed >= 2 (gate false, not timebox)", () => {
    const t0 = 2_000_000;
    setNow(t0);
    const gs: GuardState = { startedAtMs: t0, zeroRuns: 0, projectsUsed: 2 };
    // before timebox expires
    expect(shouldTransition(gs, { gateReady: false, clockMs: t0 + 1000 })).toBe("projects_cap");
  });

  it("Zero-runs counter increments on 0/0/0 and resets on non-zero", () => {
    ...
    expect(interviewChatStore.getState().background.zeroRuns).toBe(0);
  });

  it("projectsUsed initializes to 1 on background start timer", () => {
