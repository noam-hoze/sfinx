    ...
    const allEvidenceLinks = useMemo(() => {
        ...
        if (codingSummary?.jobSpecificCategories) {
            codingCategoryDefs
                ...
                .forEach((categoryDef) => {
                    const matchingKey =
                        findCategoryKeyByName(
                            codingSummary.jobSpecificCategories as Record<string, unknown>,
                            categoryDef.name
                        ) ?? categoryDef.name;
    ...
    if (loading) {
        return (
            <div className="h-screen bg-gray-50 overflow-hidden flex items-center justify-center">
                <SfinxSpinner size="lg" title="Loading candidate profile" messages={["Getting telemetry data", "Almost there"]} />
            </div>
        );
    }
