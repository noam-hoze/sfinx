...
function AboutCard({ companyName, description }: { companyName: string; description: string | null }) {
    return (
        <div className="flex flex-col items-center bg-gray-50 rounded-2xl p-8 border border-gray-100 h-full">
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-3 text-center">About {companyName}</h3>
...
function WhatWeLookFor({ cultureTags }: { cultureTags: string[] }) {
    return (
        <div className="flex flex-col items-center bg-gray-50 rounded-2xl p-8 border border-gray-100 h-full">
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-3 text-center">What we look for</h3>
            <ul className="space-y-2">
                {cultureTags.map((tag) => (
                    <li key={tag} className="flex items-center gap-2 text-base text-gray-600">
...
function MissionCard({ industry, locations, missionText }: { industry: string; locations: string[]; missionText: string }) {
    return (
        <div className="flex flex-col items-center bg-gray-50 rounded-2xl p-8 border border-gray-100 h-full">
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-3 text-center">Mission &amp; values</h3>
            <p className="text-gray-600 text-base leading-relaxed mb-3">{missionText}</p>
            {locations.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-3">
                    {locations.map((loc) => (
                        <span key={loc} className="inline-flex items-center gap-1 text-sm text-gray-500 bg-gray-50 border border-gray-100 rounded-full px-3 py-1">
