...
const STAGE_STYLES = [
    { color: "bg-blue-100 text-electric-blue border-blue-200", connector: "bg-gray-300" },
    { color: "bg-purple-100 text-purple-600 border-purple-200", connector: "bg-gray-300" },
    { color: "bg-amber-100 text-amber-600 border-amber-200", connector: "bg-gray-300" },
    { color: "bg-green-100 text-success-green border-green-200", connector: "bg-gray-300" },
];
...
function StepIcon({ index }: { index: number }) {
    ...
    return <svg className="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor">{icons[index]}</svg>;
}
...
function DesktopStage({ stage, index, isLast }: { stage: InterviewStageConfig; index: number; isLast: boolean }) {
    ...
    return (
        <div className="flex items-stretch flex-1">
            <div className="flex flex-col items-center flex-1 h-full bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-8">
                <div className={`w-16 h-16 rounded-2xl border ${style.color} flex items-center justify-center mb-4 shadow-sm`}>
                    <StepIcon index={index} />
                </div>
                <span className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">Step 0{index + 1}</span>
                <h3 className="text-lg font-bold text-gray-900 mb-2 text-center">{stage.title}</h3>
                <p className="text-base text-gray-500 text-center leading-relaxed max-w-52">{stage.shortDescription}</p>
            </div>
            {!isLast && (
...
function MobileStage({ stage, index }: { stage: InterviewStageConfig; index: number }) {
    ...
    return (
        <div className="flex gap-4 items-start">
            <div className={`w-14 h-14 rounded-xl border ${style.color} flex items-center justify-center flex-shrink-0 shadow-sm`}>
                <StepIcon index={index} />
            </div>
            <div>
                ...
                <p className="text-base text-gray-500 leading-relaxed">{stage.shortDescription}</p>
            </div>
        </div>
    );
}
...
export default function InterviewProcess({ stages }: InterviewProcessProps) {
    return (
        <section id="interview-process" className="bg-white py-24 px-6">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="hidden md:flex items-stretch justify-between gap-0 mb-16">
