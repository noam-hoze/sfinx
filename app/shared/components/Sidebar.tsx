...
export default function Sidebar() {
    ...
    // Collapsible state for AAM parent section
    const [isAamOpen, setIsAamOpen] = useState(true);
    ...
    // Keep AAM section expanded when on an AAM page
    useEffect(() => {
        if (pathname?.startsWith("/admin/policies") || pathname?.startsWith("/admin/simulation")) {
            setIsAamOpen(true);
        }
    }, [pathname]);
    ...
    return (
        ...
        >
            ...
            <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                {role === "ADMIN" ? (
                    <>
                        {/* Security & NHI Single Item */}
                        <div onClick={e => e.stopPropagation()}>
                            <Link
                                href="/admin/nhi-dashboard"
                                className={`relative flex items-center overflow-hidden transition-colors duration-150 ${
                                    ...
                                    pathname === "/admin/nhi-dashboard"
                                        ? "text-white"
                                        : "text-gray-600 hover:text-gray-900 hover:bg-white/50"
                                }`}
                                title={isCollapsed ? "Security" : undefined}
                            >
                                {pathname === "/admin/nhi-dashboard" && (
                                    <motion.div
                        ...
                        {/* Collapsible AAM Section */}
                        <div onClick={e => e.stopPropagation()} className="pt-1">
                            {/* AAM Parent Toggle Header */}
                            <button
                                type="button"
                                onClick={() => setIsAamOpen(prev => !prev)}
                                className={`w-full flex items-center justify-between transition-colors duration-150 cursor-pointer ${
                                    isCollapsed
                                        ? "justify-center w-10 h-10 mx-auto rounded-full"
                                        : "px-3 py-2.5 rounded-squircle-sm text-gray-700 hover:bg-white/50"
                                }`}
                                title={isCollapsed ? "AAM (Agentic Access Management)" : undefined}
                            >
                                ...
                                {!isCollapsed && (
                                    <svg
                                        className={`w-4 h-4 text-gray-400 transition-transform duration-200 ${isAamOpen ? 'rotate-180' : ''}`}
                                        fill="none"
                                        stroke="currentColor"
                                        viewBox="0 0 24 24"
                                    >
                            ...
                            {/* Unfoldable Sub-Items Accordion */}
                            <AnimatePresence>
                                {isAamOpen && !isCollapsed && (
                                    <motion.div
                                    ...
                                    >
                                        {/* Sub-Item 1: Intent Policies */}
                                        <Link
                                            href="/admin/policies"
                                            className={`relative flex items-center gap-2.5 px-3 py-2 rounded-squircle-sm text-xs font-semibold transition-colors duration-150 overflow-hidden ${
